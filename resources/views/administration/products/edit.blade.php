...
<section class="form__container container-fluid">
    ...
    <div class="row justify-content-center">
        <div class="col-lg-8 text-white">
            <form action="{{route('magic.edit.process',  ['id'=>$magicProduct->magic_id])}}" method="post" enctype="multipart/form-data">
                @csrf
                <div class="mb-3">
                ...
                <div class="mb-3">
                    ...
                    <option value="{{$rating->rating_id}}" @selected($rating->rating_id ==old('rating_id', $magicProduct->rating_id))>{{$rating->name}}</option>
                    @endforeach
                    </select>
                    @error('rating_id')
                ...
                <div class="mb-3">
                    ...
                    <input type="text" id="category" name="category" class="form-control"  value="{{old('category', $magicProduct->category)}}"
                    @error('category')
                ...
                <fieldset class="mb-3">
                    ...
                    <label class="me-2">
                        <input
                            ...
                            value="{{$type->types_id}}"
                            @checked(in_array($type->types_id, old('types', $magicProduct->types->pluck('types_id')->all())))
                        >
                        {{$type->name}}
                    </label>
                    @endforeach
